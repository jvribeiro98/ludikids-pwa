...
app.post('/events', authMiddleware, requireRole('COORDENACAO'), (req, res) => {
    const v = eventSchema.safeParse(req.body);
    if (!v.success)
        return res.status(400).json({ error: 'Dados inválidos' });
    const id = uid();
...
app.post('/tasks', authMiddleware, requireRole('COORDENACAO'), (req, res) => {
    const v = taskSchema.safeParse(req.body);
    if (!v.success)
        return res.status(400).json({ error: 'Dados inválidos' });
    const id = uid();
...
app.post('/payments', authMiddleware, requireRole('COORDENACAO'), (req, res) => {
    const v = paySchema.safeParse(req.body);
    if (!v.success)
        return res.status(400).json({ error: 'Dados inválidos' });
    const id = uid();
...
app.post('/babycare', authMiddleware, requireRole('PROFESSOR', 'COORDENACAO'), (req, res) => {
    const v = bcSchema.safeParse(req.body);
    if (!v.success)
        return res.status(400).json({ error: 'Dados inválidos' });
    const id = uid();
    db.prepare('INSERT INTO babycare (id,date,student_id,icons,obs,created_by) VALUES (?,?,?,?,?,?)').run(id, v.data.date, v.data.studentId, JSON.stringify(v.data.icons), v.data.obs || null, req.user.sub);
    res.json({ id });
});
